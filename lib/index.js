...
function denormalizeEntity(entityOrId, entities, schema, bag) {
  ...
  if (!bag[key].hasOwnProperty(id)) {
    ...
    var _obj = (0, _ImmutableUtils.isImmutable)(entity) ? entity : (0, _merge2.default)({}, entity);

    // Need to set this first so that if it is referenced within the call to
    // denormalizeObject, it will already exist.
    bag[key][id] = _obj;
    bag[key][id] = denormalizeObject(_obj, entities, schema, bag);
  }
  ...
  if (!bag.hasOwnProperty(key + ':' + id)) {
    var _ret = function () {
      ...
      .filter(function (attribute) {
        ...
        var itemSchema = (0, _ImmutableUtils.getIn)(schemaDefinition, [relation]);
      ...
      if (Object.keys(relationsToUpdate).length > 0) {
        returnObject = (0, _assign2.default)({}, returnObject, relationsToUpdate);
      }
